...
def process_datum_dgs_corpus(datum: ProcessedPoseDatum) -> Iterable[PoseSegmentsDatum]:
    ...
    for person in ["a", "b"]:
        if len(poses[person].body.data) > 0:
            ...
            #convert pose to float16 to save memory
            # yield {"id": f"{datum['id']}_{person}", "pose": np.float16(poses[person]), "segments": segments}
...
def get_dataset(name="dgs_corpus",
    ...
    # data1 = DiskList(tmp_dir="/local1/sign-language-translation/tmp")
    # for d in tqdm(data, desc="Processing dataset"):
    #     for e in chain.from_iterable(PROCESS_DATUM[name](d)):

    #         data1.append(e)
    # # (chain.from_iterable([PROCESS_DATUM[name](d) for d in tqdm(data, desc="Processing dataset")]))

    # data1 = list(
    #     chain.from_iterable(
    #         [PROCESS_DATUM[name](d) for d in tqdm(data, desc="Processing dataset")]
    #     )
    # )
    data1 = DiskList(tmp_dir="C:/Users/Aragorn/Programming/a4-sign-language-segmentation/tmp", cache_size=100)
    for d in tqdm(data, desc="Processing dataset"):
        for p in PROCESS_DATUM[name](d):
            data1.append(p)
    print(f"Dataset({split}) videos: {len(data1)}")
    dataset_statistics(data1)

    return PoseSegmentsDataset(data1,
