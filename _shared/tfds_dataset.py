...
def get_tfds_dataset(name,
    ...
    """return [process_datum(datum, pose_header, normalization_info, components, reduce_face) 
            ...
            """
    processed_data = DiskList(tmp_dir="C:/Users/Aragorn/Programming/a4-sign-language-segmentation/tmp", cache_size=100)
    count = 0
    for datum in tqdm(tfds_dataset, desc="Loading dataset"):
        if filter_func is None or filter_func(datum):
            processed_data.append(
                process_datum(
                    datum, pose_header, normalization_info, components, reduce_face
                )
            )
            # if count == 2:
            #     break
